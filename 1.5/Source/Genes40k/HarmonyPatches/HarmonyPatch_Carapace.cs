...
namespace Genes40k
{
    ...
    public static class StatWorker_StatOffsetFromGear_Patch
    {
        ...
        {
            if (stat == StatDefOf.MoveSpeed && val < 0f && gear.ParentHolder is Pawn_ApparelTracker pawn_ApparelTracker && pawn_ApparelTracker.pawn.genes != null && (pawn_ApparelTracker.pawn.genes.HasActiveGene(Genes40kDefOf.BEWH_BlackCarapace) || pawn_ApparelTracker.pawn.genes.HasActiveGene(Genes40kDefOf.BEWH_PrimarchAnatomy) || pawn_ApparelTracker.pawn.genes.HasActiveGene(Genes40kDefOf.BEWH_CustodesAnatomy)))
            {
                return 0f;
            }
            return val;
